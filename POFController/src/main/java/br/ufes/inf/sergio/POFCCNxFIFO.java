...
public class POFCCNxFIFO implements IOFMessageListener, IFloodlightModule, IPOFCCNxService {
	...
	public void init(FloodlightModuleContext context)
		...
		logger = LoggerFactory.getLogger(POFCCNxFIFO.class);
...
//        		}
        		...
        		CSEntry oldest = entries[0];
        		for (int i = 1; i < entries.length; i++) {
//        			System.out.println(entries[i].getName() + ", " + entries[i].getCreated() + ", " + entries[i].getUpdated());
        			if (entries[i].getCreated().before(oldest.getCreated())) {
        				oldest = entries[i];
        			}
        		}
        		removeCSEntry(oldest.getName());
        		break;
