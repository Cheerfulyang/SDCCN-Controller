...
public class POFCCNxListenerExperimento implements IOFSwitchListener {
	...
	public POFCCNxListenerExperimento() {
		logger = LoggerFactory.getLogger(POFCCNxListenerExperimento.class);
	...
	private void createInitTables(int switchId) {
		 ...
		 */
		if (pofManager.iGetProtocol("CCNx Type") == null) {
			logger.debug("Adicionando protocolo CCNx Type");
		...
		if (pofManager.iGetProtocol("CCNx Name") == null) {
			logger.debug("Adicionando protocolo CCNx Name");
			fieldList = new ArrayList<OFMatch20>();
		 ...
		 */
		if (pofManager.iGetProtocol("Ethernet sem VLAN") == null) {
			logger.debug("Adicionando protocolo Ethernet sem VLAN");
			fieldList = new ArrayList<OFMatch20>();
		...
		for (OFMatch20 mat : pofManager.iGetAllField()) {
			if (!fieldMap.containsKey(mat.getFieldName())) {
				fieldMap.put(mat.getFieldName(), mat);
			}
		}
		 ...
		 */
		logger.debug("Criando FIRT FLOW TABLE no siwtch " + switchId);
		fieldList = new ArrayList<OFMatch20>();
	...
	public void addedSwitch(IOFSwitch sw) {
		...
		logger.debug("ID = " + String.valueOf(sw.getId()));
	...
	public String getName() {
		return POFCCNxListenerExperimento.class.getSimpleName();
	}
}
